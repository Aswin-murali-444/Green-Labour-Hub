...
<?php
    if(isset($_GET['user_id'])){
        ...
        $ro=$r->fetch_assoc();
        $name=$ro['name'];
        $sql="UPDATE tbl_worker_verify SET status=0 WHERE user_id='$user_id';";
        $sql1="UPDATE tbl_login SET status=0 WHERE user_id='$user_id';";
        $conn->query($sql);
        $conn->query($sql1);
